...
export default function AdZone({ variant, slot, className = '', children }: AdZoneProps) {
  if (variant === 'skyscraper') {
    return (
      ...
      >
        <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2 py-0.5 px-2 bg-[#0A1128]/80 rounded-full inline-block mx-auto border border-[#2D305A]">
          Sponsored
        </div>
        {children || (
          <div className="flex flex-col items-center justify-center min-h-[500px] border border-dashed border-[#2D305A] rounded-xl p-2 bg-[#0A1128]/40 text-slate-500 text-xs">
            ...
            <span className="font-semibold text-white">Hot Web3 Deals</span>
            ...
            <a
              ...
              className="mt-4 px-3 py-1.5 bg-gradient-to-r from-[#00FFCC] to-[#6C5CE7] hover:opacity-95 text-[#0A1128] font-black rounded-lg text-[11px] shadow transition-all"
            >
              Claim Offer →
            </a>
          </div>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400 py-0.5 px-2 bg-[#0A1128]/80 rounded-full border border-[#2D305A]">
          Sponsored
        </span>
      </div>
      {children || (
